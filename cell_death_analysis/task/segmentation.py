...
def CellSegmentation_Original(frame: np_.ndarray,show_sg_steps: bool = False,idx: int=0,
    ...
    if show_sg_steps :
        if idx <= n_frames :
            if not os.path.exists("outputs/plot_segmentation"):
                os.makedirs("outputs/plot_segmentation")
            #save the smoothed frame image
            name_smooth ="outputs/plot_segmentation/Smoothed_frame_"+str(idx)+".png"
            img_.imsave(name_smooth,frame)
            # save the watershed segmentation image
            name_ws ="outputs/plot_segmentation/Watershed_segmentation_"+str(idx)+".png"
            img_.imsave(name_ws,crest_lines_map)
            # save the threshold-based binary segmentation
            name_thr ="outputs/plot_segmentation/threshold_based_segmentation_"+str(idx)+".png"
            img_.imsave(name_thr,segmentation)
